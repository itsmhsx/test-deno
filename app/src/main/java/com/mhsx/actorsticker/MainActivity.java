...
public class MainActivity extends Activity {
    ...
    private void buildShell() {
        ...
        title.setPadding(dp(14), dp(14), dp(14), dp(4));
        root.addView(title);

        TextView sub = text("v0.2 • Offline face scan • Actor matching • Smart crop/export • 2s minimum", 12, false);
    ...
    private void setJobProgress(int p) { runOnUiThread(() -> progress.setProgress(Math.max(0, Math.min(100, p)))); }
    ...
    private void showQueue() {
        ...
        for (int i = 0; i < videos.size(); i++) {
            line(String.format(Locale.US, "%02d  %s", i + 1, displayName(videos.get(i))));
        }
        ...
        TextView note = text("Face detection is bundled and works offline. Clips shorter than 2.0 seconds are rejected before export. Smart crop follows the average actor position inside each detected scene.", 12, false);
        note.setBackgroundColor(0xffe8f5e9);
    ...
    private void showActor() {
        ...
        content.addView(button("Clear references", v -> {
            if (!running) {
                refs.clear();
                actorTemplate = null;
                actorTemplateCount = 0;
                saveState();
                showActor();
            }
        }));
    ...
    private void showSettings() {
        clear("Settings");
        EditText threshold = edit("Actor match threshold", String.format(Locale.US, "%.2f", prefs.getFloat("threshold", 0.55f)));
        threshold.setInputType(0x2002);
        content.addView(threshold);

        EditText sample = edit("Scan interval ms", String.valueOf(prefs.getLong("sample_ms", DEFAULT_SAMPLE_MS)));
        sample.setInputType(2);
        content.addView(sample);

        EditText maxClip = edit("Max clip seconds", String.format(Locale.US, "%.1f", prefs.getLong("max_clip_ms", DEFAULT_MAX_CLIP_MS) / 1000f));
        maxClip.setInputType(0x2002);
        content.addView(maxClip);

        CheckBox keep = new CheckBox(this);
        keep.setText("Keep screen awake while processing");
        ...
        content.addView(button("Save settings", v -> {
            try {
                ...
                th = Math.max(0.20f, Math.min(0.95f, th));
                sm = Math.max(250, Math.min(3000, sm));
                long mm = Math.max(MIN_CLIP_MS, (long) (sec * 1000f));
                prefs.edit()
                        .putFloat("threshold", th)
    ...
    private void showSystem() {
        ...
        try {
            ...
            line("Codec query error: " + e.getMessage());
        }

        line("Face engine: ML Kit bundled face detector (offline)");
        line("Video engine: AndroidX Media3 Transformer 1.6.1");
    ...
    private void chooseRefs() {
        ...
        i.putExtra(Intent.EXTRA_ALLOW_MULTIPLE, true);
        i.addCategory(Intent.CATEGORY_OPENABLE);
        startActivityForResult(i, REQ_REFS);
    }
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        try {
            if (requestCode == REQ_OUTPUT) {
                ...
                if (outputTree != null) getContentResolver().takePersistableUriPermission(outputTree, flags);
            } else {
        ...
        saveState();
        if (requestCode == REQ_REFS) showActor(); else showQueue();
    }

    private void persist(Uri u, int flags) {
        try { getContentResolver().takePersistableUriPermission(u, flags); } catch (Exception ignored) {}
    }

    private void addUnique(ArrayList<Uri> list, Uri u) {
    ...
    private void buildActorTemplateAsync(boolean showResult) {
        ...
        setStatus("Building actor reference…");
        setJobProgress(0);

        worker.execute(() -> {
            try {
                FaceDetector detector = newDetector();
                ArrayList<float[]> descs = new ArrayList<>();
                int done = 0;
                for (Uri u : refs) {
                    Bitmap b = loadBitmap(u);
                    if (b != null) {
                        List<Face> faces = detect(detector, b);
                        Face best = largest(faces);
                        if (best != null) {
                            float[] d = descriptor(b, best.getBoundingBox());
                            if (d != null) descs.add(d);
                        }
                        b.recycle();
                    }
                    done++;
                    setJobProgress(done * 100 / Math.max(1, refs.size()));
                }
                detector.close();
                if (descs.isEmpty()) throw new IllegalStateException("No usable face found in reference photos");
                actorTemplate = averageNormalized(descs);
                actorTemplateCount = descs.size();
                setStatus("Actor reference ready: " + descs.size() + " face(s)");
                if (showResult) {
                    runOnUiThread(() -> new AlertDialog.Builder(this)
                            .setTitle("Actor reference ready")
                            .setMessage(actorTemplateCount + " reference face(s) accepted.\n\nNow add videos and run Analyze actor + Crop/Export.")
                ...
                actorTemplate = null;
                actorTemplateCount = 0;
                showError("Reference build failed", e);
            }
            setJobProgress(0);
        });
    }
    ...
    private float[] descriptor(Bitmap src, Rect box) {
        ...
        Bitmap crop = Bitmap.createBitmap(src, r.left, r.top, r.width(), r.height());
        Bitmap small = Bitmap.createScaledBitmap(crop, 20, 20, true);
        if (crop != small) crop.recycle();

        float[] v = new float[400];
        double mean = 0;
        int k = 0;
        for (int y = 0; y < 20; y++) {
            for (int x = 0; x < 20; x++) {
                int c = small.getPixel(x, y);
                float g = (Color.red(c) * 0.299f + Color.green(c) * 0.587f + Color.blue(c) * 0.114f) / 255f;
                v[k++] = g;
                mean += g;
            }
        }
    ...
    private float[] averageNormalized(List<float[]> ds) {
        float[] a = new float[ds.get(0).length];
        for (float[] d : ds) for (int i = 0; i < a.length; i++) a[i] += d[i];
        double n = 0;
        for (float x : a) n += x * x;
        n = Math.sqrt(n) + 1e-8;
    ...
    private void validateQueue() {
        ...
        worker.execute(() -> {
            int valid = 0, rejected = 0, unknown = 0;
            StringBuilder r = new StringBuilder();
            for (Uri u : videos) {
                long d = durationMs(u);
                if (d > 0 && d < MIN_CLIP_MS) {
                    rejected++;
                    r.append("REJECT <2s: ").append(displayName(u)).append('\n');
                } else if (d <= 0) {
                    unknown++;
                    r.append("UNKNOWN: ").append(displayName(u)).append('\n');
                } else {
                    valid++;
                    r.append(String.format(Locale.US, "OK %.1fs: %s\n", d / 1000.0, displayName(u)));
                }
            }
            final String msg = "Valid: " + valid + " • Rejected <2s: " + rejected + " • Unknown: " + unknown + "\n\n" + r;
            runOnUiThread(() -> new AlertDialog.Builder(this).setTitle("Queue validation").setMessage(msg).setPositiveButton("OK", null).show());
    ...
    private void startProcessing() {
        ...
        if (prefs.getBoolean("keep_awake", true)) getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
            ...
            try {
                ...
                for (int vi = 0; vi < videos.size() && !cancelRequested; vi++) {
                    ...
                    for (Segment s : segments) {
                        if (cancelRequested) break;
                        if (s.endMs - s.startMs < MIN_CLIP_MS) {
                            report.add("  skipped <2s");
                            continue;
                        }
                        ci++;
                        String name = safeBase(displayName(video)) + String.format(Locale.US, "_actor_%02d_%dms.mp4", ci, s.startMs);
                        setStatus("Exporting " + name);
                        File temp = exportSegmentBlocking(video, s, name);
                        if (temp != null && temp.isFile() && temp.length() > 0) {
                            copyToOutputTree(temp, name);
                            temp.delete();
                            totalClips++;
                            report.add("  OK " + name + String.format(Locale.US, "  %.1fs  score %.2f", (s.endMs - s.startMs) / 1000f, s.score));
                        }
                    }
                }
                detector.close();
    ...
    private void buildTemplateBlocking() throws Exception {
        ...
        for (Uri u : refs) {
            Bitmap b = loadBitmap(u);
            if (b != null) {
                Face f = largest(detect(detector, b));
                if (f != null) {
                    float[] d = descriptor(b, f.getBoundingBox());
                    if (d != null) ds.add(d);
                }
                b.recycle();
            }
        ...
        detector.close();
        if (ds.isEmpty()) throw new IllegalStateException("No usable face found in references");
        actorTemplate = averageNormalized(ds);
        actorTemplateCount = ds.size();
    }
    ...
    private List<Segment> scanVideo(Uri uri, FaceDetector detector, int videoIndex, int videoCount) throws Exception {
        ...
        long duration = parseLong(mr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION), -1);
        if (duration < MIN_CLIP_MS) {
            mr.release();
            return Collections.emptyList();
        }

        long sampleMs = prefs.getLong("sample_ms", DEFAULT_SAMPLE_MS);
        float threshold = prefs.getFloat("threshold", 0.55f);
        ArrayList<Hit> hits = new ArrayList<>();
        long steps = Math.max(1, (duration + sampleMs - 1) / sampleMs);
        long step = 0;

        for (long t = 0; t < duration && !cancelRequested; t += sampleMs) {
            Bitmap frame = null;
            try {
                frame = mr.getFrameAtTime(t * 1000, MediaMetadataRetriever.OPTION_CLOSEST);
                if (frame == null) { step++; continue; }
                List<Face> faces = detect(detector, frame);
                Hit best = null;
                for (Face f : faces) {
                    float[] d = descriptor(frame, f.getBoundingBox());
                    float sc = cosine(actorTemplate, d);
                    if (sc >= threshold && (best == null || sc > best.score)) {
                        Rect b = f.getBoundingBox();
                        best = new Hit(t,
                                b.centerX() / (float) frame.getWidth(),
                                b.centerY() / (float) frame.getHeight(),
                                frame.getWidth(), frame.getHeight(), sc);
                    }
                }
            ...
            step++;
            int local = (int) (step * 100 / steps);
            int overall = (int) (((videoIndex + local / 100.0) / Math.max(1, videoCount)) * 100);
            setJobProgress(overall);
        }
        mr.release();
        return buildSegments(hits, duration, sampleMs, prefs.getLong("max_clip_ms", DEFAULT_MAX_CLIP_MS));
    }
    ...
    private List<Segment> buildSegments(List<Hit> hits, long duration, long sampleMs, long maxClipMs) {
        ...
        long gap = Math.max(1400, (long) (sampleMs * 2.4));
        int i = 0;
        ...
        while (i < hits.size()) {
            ...
            long rawStart = Math.max(0, hits.get(i).t - 500);
            long rawEnd = Math.min(duration, hits.get(j).t + 1000);
            if (rawEnd - rawStart >= MIN_CLIP_MS) {
                ...
                while (chunkStart < rawEnd) {
                    ...
                    if (chunkEnd - chunkStart >= MIN_CLIP_MS) {
                        ...
                        for (int k = i; k <= j; k++) {
                            Hit h = hits.get(k);
                            if (h.t >= chunkStart && h.t <= chunkEnd) {
                                sx += h.cx * h.score;
                                sy += h.cy * h.score;
                                ss += h.score;
                                n++;
                        ...
                        float denom = Math.max(0.001f, ss);
                        out.add(new Segment(chunkStart, chunkEnd, sx / denom, sy / denom, fw, fh, ss / n));
                    }
                    chunkStart = chunkEnd;
                }
            }
    ...
    private File exportSegmentBlocking(Uri source, Segment s, String name) throws Exception {
        ...
        Crop crop = createPortraitCrop(s);
        ...
        if (!latch.await(20, TimeUnit.MINUTES)) {
            runOnUiThread(() -> { if (activeTransformer != null) activeTransformer.cancel(); });
            throw new TimeoutException("Export timeout");
        }
        if (cancelRequested) {
            if (out.exists()) out.delete();
            return null;
    ...
    private Crop createPortraitCrop(Segment s) {
        float w = Math.max(1, s.fw);
        float h = Math.max(1, s.fh);
        float aspect = 9f / 16f;
    ...
    private void cancelJob() {
        ...
        runOnUiThread(() -> {
            try { if (activeTransformer != null) activeTransformer.cancel(); } catch (Exception ignored) {}
        });
    }

    private void showError(String title, Exception e) {
        setStatus(title + ": " + e.getClass().getSimpleName());
        runOnUiThread(() -> new AlertDialog.Builder(this).setTitle(title).setMessage(e.toString()).setPositiveButton("OK", null).show());
    }
    ...
    private long durationMs(Uri u) {
        ...
        try {
            ...
            return parseLong(m.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION), -1);
        } catch (Exception e) {
            return -1;
        } finally {
    ...
    private void saveState() {
        Set<String> vs = new LinkedHashSet<>(), rs = new LinkedHashSet<>();
        for (Uri u : videos) vs.add(u.toString());
        for (Uri u : refs) rs.add(u.toString());
        SharedPreferences.Editor e = prefs.edit().putStringSet("videos", vs).putStringSet("refs", rs);
    ...
    private void restoreState() {
        for (String s : prefs.getStringSet("videos", Collections.emptySet())) {
            try { videos.add(Uri.parse(s)); } catch (Exception ignored) {}
        }
        for (String s : prefs.getStringSet("refs", Collections.emptySet())) {
            try { refs.add(Uri.parse(s)); } catch (Exception ignored) {}
        }
        String out = prefs.getString("output", null);
        if (out != null) outputTree = Uri.parse(out);
    }
    ...
    private static final class Hit {
        ...
        final int fw, fh;
        final float score;
        Hit(long t, float cx, float cy, int fw, int fh, float score) {
            this.t = t; this.cx = cx; this.cy = cy; this.fw = fw; this.fh = fh; this.score = score;
        }
    }
    ...
    private static final class Segment {
        ...
        Segment(long s, long e, float cx, float cy, int fw, int fh, float score) {
            startMs = s; endMs = e; this.cx = cx; this.cy = cy; this.fw = fw; this.fh = fh; this.score = score;
        }
    }
}
