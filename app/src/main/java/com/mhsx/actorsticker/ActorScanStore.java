...
final class ActorScanStore {
                ...
                for (int i = 0; i < cs.length(); i++) {
                    ...
                    if (hs != null) {
                        for (int j = 0; j < hs.length(); j++) {
                            ...
                            x.hits.add(new Hit(h.optInt(0), h.optLong(1),
    ...
    synchronized void save(ScanState s) throws Exception {
        ...
        o.put("format", FORMAT_VERSION); o.put("projectKey", s.projectKey);
        o.put("complete", s.complete); o.put("videoIndex", s.videoIndex);
        o.put("nextMs", s.nextMs); o.put("sampleMs", s.sampleMs);
        o.put("selectedClusterId", s.selectedClusterId);
        JSONArray cs = new JSONArray();
        for (Cluster x : s.clusters) {
            JSONObject c = new JSONObject();
            c.put("id", x.id); c.put("count", x.count); c.put("qualitySum", x.qualitySum);
            c.put("centroid", encodeFloats(x.centroid)); c.put("lastSeenVideo", x.lastSeenVideo); c.put("lastSeenMs", x.lastSeenMs);
            JSONArray ts = new JSONArray(); for (String t : x.thumbs) ts.put(t); c.put("thumbs", ts);
            JSONArray tq = new JSONArray(); for (Float q : x.thumbQualities) tq.put(q); c.put("thumbQualities", tq);
            JSONArray vs = new JSONArray(); for (Integer v : x.videosSeen) vs.put(v); c.put("videosSeen", vs);
            JSONArray hs = new JSONArray();
            for (Hit h : x.hits) {
                JSONArray a = new JSONArray();
                a.put(h.videoIndex); a.put(h.t); a.put(h.cx); a.put(h.cy); a.put(h.fw); a.put(h.fh); a.put(h.score); a.put(h.quality);
                hs.put(a);
            }
            c.put("hits", hs); cs.put(c);
        }
        o.put("clusters", cs);
        File dir = projectDir(s.projectKey), temp = new File(dir, "index.json.tmp"), dest = new File(dir, "index.json");
        try (Writer w = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(temp), "UTF-8"))) { w.write(o.toString()); }
    ...
    String saveThumbnail(String key, int clusterId, int serial, Bitmap bitmap) throws Exception {
        ...
        try (OutputStream out = new BufferedOutputStream(new FileOutputStream(f))) {
            if (!bitmap.compress(Bitmap.CompressFormat.WEBP, 86, out)) throw new IOException("Thumbnail encode failed");
        }
        return name;
    }

    File thumbnailFile(String key, String relative) { return new File(projectDir(key), relative); }
    void deleteThumbnail(String key, String relative) { try { new File(projectDir(key), relative).delete(); } catch (Throwable ignored) {} }
    void clearProject(String key) { deleteRecursively(new File(root, key)); }
    void clearAll() { File[] fs = root.listFiles(); if (fs != null) for (File f : fs) deleteRecursively(f); }
    long cacheBytes() { return bytes(root); }

    /** Removes oldest project directories until the private cache is under the requested limit. */
    long trimToBytes(long limit) {
        limit = Math.max(64L * 1024 * 1024, limit);
        long total = cacheBytes();
        if (total <= limit) return total;
        File[] projects = root.listFiles(File::isDirectory);
        if (projects == null) return total;
        Arrays.sort(projects, Comparator.comparingLong(File::lastModified));
        for (File p : projects) {
    ...
    private static long bytes(File f) {
        ...
        long n = 0; File[] xs = f.listFiles(); if (xs != null) for (File x : xs) n += bytes(x); return n;
    }
    ...
    private static float[] decodeFloats(String s) {
        ...
        try {
            ...
            float[] a = new float[raw.length / 4]; for (int i = 0; i < a.length; i++) a[i] = b.getFloat(); return a;
        } catch (Throwable ignored) { return null; }
    }

    static final class ScanState {
        final String projectKey; final ArrayList<Cluster> clusters = new ArrayList<>();
        boolean complete; int videoIndex; long nextMs; long sampleMs = 900L; int selectedClusterId = -1;
        ScanState(String projectKey) { this.projectKey = projectKey; }
        Cluster byId(int id) { for (Cluster c : clusters) if (c.id == id) return c; return null; }
    }

    static final class Cluster {
        final int id; int count; double qualitySum; float[] centroid;
        ...
        int lastSeenVideo = -1; long lastSeenMs = -1L;
        Cluster(int id) { this.id = id; }
        float avgQuality() { return count <= 0 ? 0f : (float) (qualitySum / count); }
        double smartScore() { double coverage = 1.0 + Math.min(2.0, videosSeen.size() * 0.22); return count * (0.45 + avgQuality()) * coverage; }
        void updateCentroid(float[] d) {
            if (centroid == null) { centroid = d.clone(); FaceEngine.normalize(centroid); return; }
            if (centroid.length != d.length) { centroid = d.clone(); FaceEngine.normalize(centroid); return; }
            float alpha = 1f / Math.min(34f, Math.max(2f, count));
            for (int i = 0; i < centroid.length; i++) centroid[i] = centroid[i] * (1f - alpha) + d[i] * alpha;
            FaceEngine.normalize(centroid);
        }
    }

    static final class Hit {
        final int videoIndex; final long t; final float cx, cy; final int fw, fh; final float score, quality;
        Hit(int videoIndex, long t, float cx, float cy, int fw, int fh, float score, float quality) {
            this.videoIndex = videoIndex; this.t = t; this.cx = cx; this.cy = cy; this.fw = fw; this.fh = fh; this.score = score; this.quality = quality;
        }
    }
}
