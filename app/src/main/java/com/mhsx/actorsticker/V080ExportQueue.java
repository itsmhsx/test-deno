...
final class V080ExportQueue {
    ...
    V080ExportQueue(Context c){file=new File(c.getFilesDir(),"v080_export_queue.json");}
    ...
    synchronized void save(List<Job> xs){try{JSONArray a=new JSONArray();for(Job j:xs)a.put(j.json());File tmp=new File(file.getParentFile(),file.getName()+".tmp");try(Writer w=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(tmp),"UTF-8"))){w.write(a.toString());}if(file.exists())file.delete();tmp.renameTo(file);}catch(Throwable ignored){}}
    synchronized void append(List<Job> add){ArrayList<Job> xs=load();xs.addAll(add);save(xs);}
    ...
    synchronized Job claimNext(){ArrayList<Job> xs=load();for(Job j:xs){if(PENDING.equals(j.state)){j.state=RUNNING;j.attempts++;save(xs);return copy(j);}}return null;}
    ...
    synchronized void retryFailed(){ArrayList<Job> xs=load();for(Job j:xs)if(FAILED.equals(j.state)){j.state=PENDING;j.error="";}save(xs);}
